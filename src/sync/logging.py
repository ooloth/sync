...
def setup_logging(verbose: bool = False, job_name: str | None = None) -> None:
    ...
    # Processor to strip "sync." prefix from logger names
    def strip_logger_prefix(logger, method_name, event_dict):
        """Strip 'sync.' prefix from logger names for cleaner output."""
        if "logger" in event_dict and event_dict["logger"].startswith("sync."):
            event_dict["logger"] = event_dict["logger"][len("sync.") :]
        return event_dict
    ...
    structlog.configure(
        processors=[
            ...
            # Strip sync. prefix from logger names
            strip_logger_prefix,
