...
from sync.io.youtube import YouTubeAuth, YouTubeClient, YouTubeSubscription, create_client
...
def test_client_initializes_youtube_resource(mock_credentials, mock_youtube_resource):
    ...
    with patch("sync.io.youtube.client.build", return_value=mock_youtube_resource) as mock_build:
...
def test_list_subscriptions_handles_single_page(mock_credentials, mock_youtube_resource):
    ...
    with patch("sync.io.youtube.client.build", return_value=mock_youtube_resource):
        client = YouTubeClient(mock_credentials)
...
def test_create_client_without_auth_uses_1password(mock_credentials, mock_youtube_resource):
    ...
    with (
        patch("sync.io.youtube.client.create_auth_from_1password") as mock_create_auth,
        patch("sync.io.youtube.client.build", return_value=mock_youtube_resource),
    ):
        mock_auth = Mock(spec=YouTubeAuth)
        mock_auth.get_credentials.return_value = mock_credentials
