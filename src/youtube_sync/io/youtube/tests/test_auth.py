...
def test_get_tokens_file_falls_back_to_home(monkeypatch, tmp_path):
    ...
    with patch("subprocess.run", side_effect=subprocess.CalledProcessError(1, "git")):
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            assert result == tmp_path / ".youtube_sync" / "oauth_access_token.json"
...
def test_youtube_auth_get_credentials_uses_valid_cache(
    ...
    with patch("youtube_sync.io.youtube.auth.Credentials") as mock_creds_class:
...
def test_youtube_auth_refreshes_expired_token(tmp_path, mock_client_config, mock_credentials):
    ...
    with (
        patch("youtube_sync.io.youtube.auth.get_secret", return_value="test_refresh_token"),
...
def test_youtube_auth_handles_missing_cache(tmp_path, mock_client_config):
    ...
    with (
        patch("youtube_sync.io.youtube.auth.get_secret", return_value="test_refresh_token"),
        patch("youtube_sync.io.youtube.auth.Credentials") as mock_creds_class,
        patch("youtube_sync.io.youtube.auth._cache_access_token"),
    ):
